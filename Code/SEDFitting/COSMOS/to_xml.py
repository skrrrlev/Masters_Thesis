...
def main():
    ...
    for i,group in enumerate(target_ids_grouped):
        ...
        root = ET.Element("catalogue", name=f"CAT-COSMOS-{i+1}")
        ...
        for target in targets:
            ...
            ET.SubElement(target_root,"id").text = f'{target}'
        ...
        spec_source = ET.SubElement(sources_element,"source",name="spec files")
        ...
        for specfile in specfiles:
            ...
            for observation in specfile.observations:
                ...
                attrib = {"target":f"{specfile.id}","name":f"{observation.column.replace('_','/')}","unit":"mJy","type":"F"}
        ...
        deblended_source = ET.SubElement(sources_element,"source",name="deblended fits file")
        ...
        for observation in ir_data:
            ...
            if observation.type == ColumnType.FILTER:
                ...
                attrib = {"target":f"{observation.target_id}","name":f"{observation.column.replace('_','/')}","unit":"mJy","type":"F"}
                obs_element = ET.SubElement(deblended_source,"observation",attrib=attrib)
                ET.SubElement(obs_element,"flux").text = str(observation.flux)
                ET.SubElement(obs_element,"error").text = str(observation.flux_error)
                ET.SubElement(obs_element,"typeval").text = str(observation.filter.stardust_code)
                ...
                attrib = {"target":f"{observation.target_id}","name":f"{observation.column.replace('_','/')}","unit":"mJy","type":"E"}
                obs_element = ET.SubElement(deblended_source,"observation",attrib=attrib)
                ET.SubElement(obs_element,"flux").text = str(observation.flux)
                ET.SubElement(obs_element,"error").text = str(observation.flux_error)
